        ...
        <nav className="w-full py-6 flex justify-between items-center navbar">
            ...
            <ul className="list-none sm:flex hidden justify-between items-center">
                {navLinks.map((nav, idx) => (
                    <li
                        key={nav.id}
                        className={`cursor-pointer font-poppins font-normal text-[16px] text-white ${idx === navLinks.length - 1 ? "mr-0" : "mr-10"
                            } `}
                    >
                ...
                >
                    <ul className="list-none flex flex-col justify-between items-center flex-1">
                        {navLinks.map((nav, idx) => (
                            <li
                                key={nav.id}
                                className={`cursor-pointer font-poppins font-normal text-[16px] text-white ${idx === navLinks.length - 1 ? "mr-0" : "mb-4"
                                    } `}
                            >
                                <a href={`#${nav.id}`}>{nav.title}</a>
                            </li>
